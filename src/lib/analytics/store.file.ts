...
/**
 * Where the events live when no database is configured.
 *
 * A JSON file under `.data/`, kept as the fallback for a machine with no
 * MONGODB_URI — the whole dashboard still runs with `npm run dev` and nothing
 * else, which is what made this worth keeping rather than deleting.
 *
 * It is NOT suitable for Vercel: the filesystem there is ephemeral and is not
 * shared between instances, so a row written by one request is invisible to
 * the next and gone at the next deploy. store.ts routes to Mongo whenever a
 * URI is set, and says so loudly when there is not one.
 *
